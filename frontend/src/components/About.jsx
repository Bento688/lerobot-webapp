...
const About = () => {
  ...
  const AccentBar =
    "w-1.5 h-8 bg-primary rounded-full shadow-[0_0_15px_rgba(255,99,71,0.5)]";
  const TextPrimary =
    "bg-clip-text text-transparent bg-gradient-to-r from-primary to-red-500";

  // NOTE: BoxStyle includes the flow boxes' hover effect (already approved)
  const BoxStyle =
    "p-5 border border-base-300 rounded-xl bg-base-200 shadow-xl h-full transition-transform duration-300 hover:scale-[1.03] cursor-pointer";

  return (
    <div className="text-base-content px-6 max-w-4xl mx-auto space-y-20 py-2">
      {/* HEADER SECTION (SIMPLIFIED INTRODUCTION) */}
      <motion.section {...fadeIn} className="text-center space-y-4 pt-4">
        ...
        <p className="text-xl text-base-content/70 max-w-3xl mx-auto">
          We combine <strong>Artificial Intelligence (AI)</strong> and{" "}
          <strong>Robotics</strong> to solve a simple, but important problem:
          teaching a robot to look at tomatoes and know which ones are ready to
          be picked.
        </p>
      </motion.section>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-4xl font-poppins font-semibold">
            What Is TomaTVLA's Goal?
          </h2>
        </div>
        <div className="text-base-content/70 space-y-4 text-lg">
          <p>
            Our project’s main goal is to create a robot system that can perform
            a complex real-world task:{" "}
            <strong>automated sorting in agriculture.</strong> Imagine working
            on a farm or in a food processing plant. Sorting fruits like
            tomatoes takes a lot of time and human effort, and mistakes can be
            costly.
          </p>
          <p className="font-semibold text-base-content">
            We trained the LeRobot SO101 arm injected with SmolVLA VLA model to
            be a smart picker. It only picks up{" "}
            <strong>ripe (red) tomatoes</strong> and leaves the{" "}
            <strong>unripe (green) ones</strong>, placing the ripe ones neatly
            into a collection box. It does this by using cameras installed to
            its surrounding, and the processing of the SmolVLA model.
          </p>
          <p>
            The name <strong>TomaTVLA</strong> stands for{" "}
            <strong>Tomat Vision, Language ,Action</strong>, reflecting our
            focus on teaching the robot how to <strong>see</strong> and{" "}
            <strong>learn</strong> to do its job.
          </p>
        </div>
      </motion.section>
      ...
      >
        <div className="flex items-center gap-3">
          <div className={AccentBar}></div>
          <h2 className="text-4xl font-poppins font-semibold">
            How Does the Robot "See" and Sort?
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {/* Component 1: The Arm */}
          <div
            className={`p-5 border border-base-300 rounded-xl bg-base-200 shadow-xl space-y-3`}
          >
            <h3 className="text-2xl font-poppins font-bold flex items-center gap-2">
              <Lightbulb className="text-primary w-6 h-6" /> The Robotic Arm
            </h3>
            <p className="text-xl text-base-content/70">
              We use the <strong>LeRobot SO101</strong>, a versatile,
              six-jointed robotic arm (like a human arm with six major
          ...
          >
            <h3 className="text-2xl font-poppins font-bold flex items-center gap-2">
              <Zap className="text-primary w-6 h-6" /> The AI Brain (SmolVLA &
              YOLO)
            </h3>
            <p className="text-base-content/70">
              The core of the robot's intelligence is <strong>SmolVLA</strong>,
              a <strong>Vision-Language-Action</strong> model that serves as the
              main "brain" for reasoning and robotic control. To enhance visual
              precision, we layer a custom-trained{" "}
              <strong>YOLO (You Only Look Once)</strong> model on top of it.
              While
              <strong>
                {" "}
                SmolVLA handles the complex decision-making and manipulation
              </strong>{" "}
              tasks, <strong>YOLO acts as a specialized visual cortex</strong>,
              instantly pinpointing the exact coordinates of red (ripe) versus
              green (unripe) tomatoes to guide SmolVLA's actions.
            </p>
          </div>
        </div>

        <p className="text-base-content/70 text-center italic pt-4">
          The robot is trained to make a simple, binary decision:{" "}
          <strong>Ripe (Pick)</strong> or <strong>Unripe (Ignore)</strong>.
        </p>
      </motion.section>
      ...
      >
        ...
        <div className="grid md:grid-cols-5 gap-4 items-stretch justify-center pt-2">
          ...
          <div
            className={`${BoxStyle} col-span-2 md:col-span-1 border-primary/50 flex flex-col justify-center`}
          >
          ...
          <div
            className={`${BoxStyle} col-span-1 border-primary/50 flex flex-col justify-center`}
          >
            <CheckCircle className="w-8 h-8 mx-auto text-primary" />
            <h3 className="font-bold text-xl mt-2">3. Action</h3>
            <p className="text-sm text-base-content/60 mt-1">
              The robot moves and executes the task based on the command and
              vision data.
            </p>
          </div>
        </div>
      ...
      >
        <div className="flex items-center gap-3">
          <div className={AccentBar}></div>
          <h2 className="text-4xl font-poppins font-semibold">
            The Control Loop: Simple Instructions to Real Action
          </h2>
        </div>
        <p className="text-base-content/70 text-lg">
          How does your typed message actually make the heavy robot arm move? It
          follows a clear, three-step process:
        </p>
      ...
      >
        <div className="flex items-center gap-3">
          <div className={AccentBar}></div>
          <h2 className="text-4xl font-poppins font-semibold">
            Our Mission: Making AI Robotics Accessible
          </h2>
        </div>
        <p className="text-base-content/70 text-lg leading-relaxed">
          TomaTVLA is more than just a tomato sorter; it's a demonstration of
          how modern technology can be applied to practical problems. Our
          mission is to{" "}
          <strong>make AI robotics easy to understand and use</strong>. By
          combining a simple chat interface with a smart robotic arm and live
          video feed, our goal is to support the agricultural world through
          practical automation. Instead of relying on time-consuming manual
          labor, tasks like picking produce can be handled by the robot, making
          the workflow faster, more efficient, and easier for farmers.
        </p>
      </motion.section>
    </div>
  );
