...
try:
print("LAZY LOADING: Loading YOLO model 'models/best.pt'...") # Ensure you have a 'models' folder with 'best.pt' inside
...
def data*url_to_frame(data_url: str):
...
try:
while True: # 1. Receive User Message
...
"""Reads from the camera and streams MJPEG.""" # Try index 1 (External) first, then 0 (Internal)
